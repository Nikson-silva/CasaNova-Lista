-- Preserve reservation history while allowing an active reservation to be cancelled.
alter table public.reservations
  add column if not exists cancelled_at timestamptz;

comment on column public.reservations.cancelled_at is
  'NULL for an active reservation; timestamp recorded when an administrator cancels it.';

-- Atomically cancels the single active reservation and releases its gift.
...
comment on function public.cancel_gift_reservation(uuid)
is 'Atomically cancels the active reservation for a gift and marks the gift available.';

revoke execute on function public.cancel_gift_reservation(uuid)
from public, anon, authenticated;

grant execute on function public.cancel_gift_reservation(uuid)
to service_role;
